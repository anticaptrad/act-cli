...
export function loadFrom(lookup: (key: string) => string | undefined): CliEnvValues {
  return {
    ...
    act_cli_bind: nonEmpty(lookup("ACT_CLI_BIND")) ?? "127.0.0.1:9090",
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const act_access_token = pick(["ACT_ACCESS_TOKEN"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (act_access_token !== undefined) out["ACT_ACCESS_TOKEN"] = act_access_token;
  const act_api_url = pick(["ACT_API_URL"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (act_api_url !== undefined) out["ACT_API_URL"] = act_api_url;
  const act_cli_bind = pick(["ACT_CLI_BIND"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "127.0.0.1:9090");
  if (act_cli_bind !== undefined) out["ACT_CLI_BIND"] = act_cli_bind;
  return out;
}
