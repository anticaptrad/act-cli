...
pub struct CliEnvValues {
    pub act_access_token: Option<String>,
    pub act_api_url: Option<String>,
    pub act_cli_bind: String,
}

/// Pure: resolve values from an explicit lookup.
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        act_access_token: lookup("ACT_ACCESS_TOKEN").filter(|value| !value.is_empty()),
        act_api_url: lookup("ACT_API_URL").filter(|value| !value.is_empty()),
        act_cli_bind: lookup("ACT_CLI_BIND").filter(|value| !value.is_empty()).unwrap_or_else(|| "127.0.0.1:9090".to_string()),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let act_access_token = pick(&["ACT_ACCESS_TOKEN"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = act_access_token {
        out.insert("ACT_ACCESS_TOKEN".to_string(), value);
    }
    let act_api_url = pick(&["ACT_API_URL"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = act_api_url {
        out.insert("ACT_API_URL".to_string(), value);
    }
    let act_cli_bind = pick(&["ACT_CLI_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("127.0.0.1:9090"));
    if let Some(value) = act_cli_bind {
        out.insert("ACT_CLI_BIND".to_string(), value);
    }
    Ok(out)
}
